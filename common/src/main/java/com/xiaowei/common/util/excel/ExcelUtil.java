...
public class ExcelUtil {
	...
	public static void exportExcelX(String title, Map<String, String> headMap, JSONArray jsonArray, String datePattern,
		...
		headerStyle.setBorderTop(HSSFCellStyle.BORDER_THIN);
		headerStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);
		headerStyle.setFillForegroundColor(HSSFColor.WHITE.index);  //单元格样式添加这个
		...
		cellFont.setBoldweight(HSSFFont.BOLDWEIGHT_NORMAL);
		cellStyle.setFont(cellFont);
		cellStyle.setFillForegroundColor(HSSFColor.WHITE.index);  //单元格样式添加这个
		// 生成一个(带标题)表格
		Sheet sheet = workbook.createSheet();
		// 设置列宽
		int minBytes = colWidth < DEFAULT_COLOUMN_WIDTH ? DEFAULT_COLOUMN_WIDTH+10 : colWidth;// 至少字节数
		int[] arrColWidth = new int[headMap.size()];
		// 产生表格标题行,以及设置列宽
		String[] properties = new String[headMap.size()];
		String[] headers = new String[headMap.size()];
	...
	public static void main(String[] args) throws IOException {
		 ...
		 */
		//
		OutputStream outXlsx = new FileOutputStream("D://英雄时刻//b.xlsx");
		log.info("正在导出xlsx....");
		Date d2 = new Date();
		ExcelUtil.exportExcelX(title, headMap, ja, null, 0, outXlsx);
		log.info("共" + count + "条数据,执行" + (System.currentTimeMillis() - d2.getTime()) + "ms");
		outXlsx.close();
