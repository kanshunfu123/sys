package com.xiaowei.mana.mapper.Util;
...
//        比较  现在的时间 比 设定的时间 之前  返回的类型是Boolean类型
     ...
     * @return
     */
    public static String formatLocalDateTimeToString(Date localDateTime, String format) {
        try {
            Instant instant = localDateTime.toInstant();
            ZoneId zoneId = ZoneId.systemDefault();
            LocalDateTime localDateTimeaaa = instant.atZone(zoneId).toLocalDateTime();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern(format);
            return localDateTimeaaa.format(formatter);
